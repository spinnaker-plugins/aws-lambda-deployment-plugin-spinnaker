...
public class LambdaCreateTaskTest {
    ...
    @InjectMocks
    private LambdaCreateTask lambdaCreateTask;
    ...
    void init(@WiremockResolver.Wiremock WireMockServer wireMockServer, @WiremockUriResolver.WiremockUri String uri) {
        ...
        Mockito.when(lambdaCloudDriverUtilsMock.getInput(stageExecution, LambdaDeploymentInput.class)).thenReturn(ldi);
        LambdaGetInput lgi = LambdaGetInput.builder()
                .build();
        Mockito.when(lambdaCloudDriverUtilsMock.getInput(stageExecution, LambdaGetInput.class)).thenReturn(lgi);
    ...
    public void execute_InsertLambda_SUCCEEDED(){
        Mockito.when(lambdaCloudDriverUtilsMock.retrieveLambdaFromCache(stageExecution,false)).thenReturn(null);
    ...
    public void execute_UpsertLambda_SUCCEEDED(){
        ...
        LambdaDefinition ld = LambdaDefinition.builder()
                .revisions(revisions)
                .build();
        Mockito.when(lambdaCloudDriverUtilsMock.retrieveLambdaFromCache(stageExecution,false)).thenReturn(ld);
        assertEquals(ExecutionStatus.SUCCEEDED, lambdaCreateTask.execute(stageExecution).getStatus());
    }
    ...
    public void execute_InvalidUpsertLambdaInput_TERMINAL(){
        ...
        LambdaDefinition ld = LambdaDefinition.builder()
                ...
                .build();
        Mockito.when(lambdaCloudDriverUtilsMock.retrieveLambdaFromCache(stageExecution,false)).thenReturn(ld);
        Mockito.when(lambdaCloudDriverUtilsMock.validateUpsertLambdaInput(Mockito.any(), Mockito.anyList())).thenReturn(false);

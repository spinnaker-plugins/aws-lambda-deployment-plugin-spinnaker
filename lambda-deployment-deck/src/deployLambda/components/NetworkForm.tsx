...
export function NetworkForm( props: IFormikStageConfigInjectedProps ) { 
  ...
  const availableVpcs = values.account && values.region && fetchVpcsStatus !== 'PENDING' ?
    fetchVpcsResult
  ...
  const availableSGs = values.account && values.region && values.vpcId && fetchSGsResult.length !== 0 ?
      ...
      <div className="form-group">
        ...
        { dedupedSubnets.length === 0 ?
          ...
          <div className="col-md-6">
            <TetheredSelect
              multi={true}
              options={dedupedSubnets
                .map((s: ISubnet) => toSubnetOption(s))
              }        
